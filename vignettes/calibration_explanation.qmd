...
```{r}
# Look for "inst/models" inside the installed package
# model_dir <- system.file("models", package = "epiworldRcalibrate", mustWork = FALSE)
# 
# # Initialize only if the directory actually exists
# if (nzchar(model_dir) && dir.exists(model_dir)) {
#   init_bilstm_model(model_dir)
# } else {
#   message("⚠️ BiLSTM model directory not found in installed package; skipping LSTM initialization.")
# }

# Use vignette-safe prediction that handles missing models gracefully
lstm_predictions <- vignette_safe_predict(incidence_ts, n = true_params$n, recov = true_params$recovery_rate)
